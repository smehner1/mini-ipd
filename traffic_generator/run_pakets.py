...
def run_pakets(
    ...
    interval_time (float): time within all pakets need to be sended
    net (int): AS the traffic originates
    router (int): identification number of router in AS
    dir (str): path to connection files (normally mounted directory of hosts)
    verbose (bool) = False: set True to get debug output
    noise (float) = 0.0: part of traffic that gets noisy
    overflow (bool) = False: set True if this AS is included in an overflow event
    '''
